...
//!   The `read` tool — read a file with binary detection + pagination.
//!   Ports `src/tools/read.ts`: `~/` expansion, relative→cwd resolution,
//!   binary = null byte in the first 8KB → `[Binary file: ...]`, page
//!   defaults (2000 lines / 100KB), `[Showing lines X-Y of Z...]` footer.
//!
//! DESIGN PATTERNS USED:
//!   * Strategy — implements `AgentTool`; stays `Parallel` (read-only, the
//!     only builtin allowed concurrent batches in TS).
//!
//! TYPES PRESENT IN THIS FILE:
//!   * `ReadTool` — `new(cwd)`; pure helper `read_page()`.
//!
//! HOW TO USE (example):
//! ```rust,no_run
...
impl AgentTool for ReadTool {
    ...
    async fn execute(&self, _id: &str, args: serde_json::Value) -> anyhow::Result<ToolOutput> {
        ...
        // Binary sniff: null byte in the first 8KB (TS `read.ts` rule).
        ...
        // Byte cap: slice first (TS slices the page to 100KB).
